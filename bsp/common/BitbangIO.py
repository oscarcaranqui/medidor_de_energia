...
class I2C(_BitBangIO):
    ...
    def __init__(
        self, scl: int, sda: int, *, frequency: int = 400000, timeout: float = 1
    ) -> None:
    ...
    def _scl_low(self) -> None:
        GPIO.setup(self._scl, GPIO.OUT, initial=GPIO.LOW)

    def _sda_low(self) -> None:
        GPIO.setup(self._sda, GPIO.OUT, initial=GPIO.LOW)
    ...
    def _write_byte(self, byte: int) -> bool:
        ...
        self._scl_low()
        GPIO.setup(self._scl, GPIO.IN)  # SDA may go high, but SCL is low
        self._wait()

        self._scl_release()
        self._wait()
        ack = GPIO.input(self._sda)  # read the ack
    ...
    def _read_byte(self, ack: bool = False) -> int:
        ...
        for _ in range(8):
            self._scl_release()
            self._wait()
            data = (data << 1) | int(GPIO.input(self._sda))
            self._scl_low()
            self._wait()
    ...
    def _write(self, address: int, buffer: bytes, transmit_stop: bool) -> None:
        self._start()
